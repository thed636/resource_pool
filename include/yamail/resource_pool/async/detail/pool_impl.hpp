...
void pool_impl<V, I, Q>::get(Callback&& call, time_traits::duration wait_duration) {
    ...
    if (_disabled) {
        lock.unlock();
        async_call([call = std::forward<Callback>(call)] {
            call(make_error_code(error::disabled), list_iterator());
        });
    } else if (alloc_resource(lock, std::forward<Callback>(call))) {
        return;
    } else if (fit_capacity()) {
        reserve_resource(lock, std::forward<Callback>(call));
    } else {
        lock.unlock();
...
bool pool_impl<V, I, Q>::alloc_resource(unique_lock& lock, Callback&& call) {
    while (!_available.empty()) {
        ...
        ++_used_size;
        lock.unlock();
        async_call([call = std::forward<Callback>(call), res_it] {
